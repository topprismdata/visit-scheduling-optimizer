...
>    - **Layer 1 全月排历**：R2′ 原生列生成（`r2_alns` 整店换星期几）+ 对偶闭环列生成终解。**唯一基线 = 原分配 + CP-SAT 日内最优排序**（全办 4,144.3 km；SRP 打印序里程无业务意义、废止引用）。全办实测：**4,144.3 → 3,618.5 km（−12.69%，净省 525.8 km/月）**；10/10 线零分裂（每店全月一星期几）· 零越界（走廊 [K_min,K_max]）· 结构保证成立；
