...
> 2. **ALNS v4 通用稳定化层**：支持 $\lambda$（稳定性/改动成本）与 $\mu$（每日工作量均衡度）双旋钮帕累托调整，实现"微调 3 家店净省 23 公里"的高性价比调整；
> 3. **Agent 动态实时调度副驾**：针对现场实际走访中高达 27.8%~34.6% 的突发临时插单需求，基于近 3 年学术顶刊（2024 Transportation Science 亚马逊冠军方案）的**沿街走廊一维投影与顺路拼接算法**，在 < 2 毫秒内完成在途插单，全月直接砍掉 **52.0%（−642.5 km）** 的无效折返骑行！
...
|---|:---:|:---:|---|
| **对比 1：人类业代实际走法（现状）** | **1,234.7 km** | — | 人类凭直觉边走边加，折返跑、逆行绕路极其严重 |
| **对比 2：事后理论全局最优（静态上限）** | **578.4 km** | 20~35 ms | 事后全量已知的理论极限下界 |
| **对比 3：Agent 动态在途规划（实时副驾）** | **592.2 km** | **< 2.5 ms** | **相比人类净省 642.5 km（直接削减 52.0%!）**，几乎持平理论极限！ |
...
### 5. V4 多目标帕累托方案生成器 (MO-ALNS v4)

不同于传统单目标优化（仅最小化里程），V4 使用 **NSGA-II + ALNS 混合**在四个维度上同时搜索帕累托前沿：

| 维度 | 定义 | 业务含义 |
|---|---|---|
| f₁ 总里程 | Σ day_km | "省多少油钱" |
| f₂ 改动量 | 相对原始计划被挪的店数 | "业代要适应多少变化" |
| f₃ 均衡度 | 每日拜访量变异系数 CV | "有没有哪天特别累" |
| f₄ 走廊连贯 | 跨街区边占比 | "路线是否顺路不折返" |

09 线路实测（2058 代，25 个非支配解）：

| 方案 | 里程 (km) | 改动店数 | CV 均衡度 | 跨区率 |
|---|:---:|:---:|:---:|:---:|
| 🔥 激进型 | **346.2** | 19 店 (11.7%) | 0.1449 | 35.4% |
| ⭐ 推荐型(膝点) | 397.5 | 17 店 (10.4%) | **0.1095** | 35.6% |
| ⚖️ 均衡型 | 464.1 | 59 店 (36.2%) | **0.0127** | 37.6% |
| 🔒 保守型 | 362.6 | **0 店 (0%)** | 0.1455 | 35.0% |
...
| `docs/COMPARISON_REPORT_V4_PARETO.md` | V4 双旋钮帕累托调节（λ/μ 扫描 + 贪心回退） |
