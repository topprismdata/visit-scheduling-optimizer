...
本项目对"拜访频次"的理解经历三次推翻重建，每次都由**用户纠错 + 原始数据全量核验**驱动：

1. **月频次（v1，错）**：把"每店月内 f 行"当作合同，MOVE 枚举 C(k,f) 全组合。放行了 {周1,周4} 这类 SRP 永不生成的组合。用户纠正："频次单位是周"。
2. **均匀节拍（v2，仍松弛）**：f 次拜访圆形等分 k 槽位，跨度 ∈ {⌊k/f⌋,⌈k/f⌉}。原始计划 1,524 店零违例——但它仍是松弛超集（k=5,f=3 放行 5 种组合，SRP 只生成 {0,2,4}）。教训：**零违例只能证明"规则 ≥ 数据"，不能证明"规则 = 数据"**。
3. **合同-相位（v3，定稿）**：合同 ∈ {周访, 双周访(φ∈{0,1})}，相位 = ISO 自然周 mod 2，`服务周`字段 = ISO 周 mod 4（1,524 家全部吻合）。62 家"f=3"实为双周奇相位挂 k=5 星期几。用户质疑"不应该存在 f=3"——对合同层语义的直觉先于数据归纳。

**方法论**：业务语义以 SRP 数据为零例外事实源；语义假设必须过"原始计划精确重构（集合相等）"才能进模型；重构相等（而非仅不违例）才是定稿标准。

**同一日确立的配套裁定**：
- **仓库三层抽离**：判据用 Google 标准（变化耦合/所有权/Hyrum's Law）而非 LOC——主线仅 6,862 行但仓库叠了三代项目 33k 行遗产；增量解耦（归档→实验归置→`core/contract.py` 立缝），不做 big-bang；
