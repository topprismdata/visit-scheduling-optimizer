...
> **⚠ 已实测**：生产距离矩阵**非对称**（OSM 单行道所致；02/09/11 线 max|D−Dᵀ| 分别为 0.83/1.83/5.51 km）。当前 2-opt 在非对称矩阵上接受判据失真——**只影响搜索效率，不影响交付正确性**（结果路线的 km 始终按有向矩阵精确重算；cpsat 门控路径另行已证最优）。修复选项：directed-safe 2-opt（段内全重算）或改用 relocate 类算子——排期中。
...
- NN 复杂度 O(n²)；2-opt 每轮 O(n²)；n=35 实测毫秒级——速度对比的基线点。
