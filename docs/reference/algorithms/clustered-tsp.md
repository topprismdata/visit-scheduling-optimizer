...
Amazon Last-Mile 冠军方法（Cook/Held/Helsgaun）的移植尝试：**跨区块边加大罚项 M**（10×max(D)）把"同区块店尽量连续拜访"的偏好编译进 TSP 目标。动机：业代跨区穿行成本高（路网/管理原因），希望按行政区块聚类拜访。

```
D2[i][j] = D[stores[i]][stores[j]] + (M if zone(i) != zone(j) else 0)
整体求解 TSP（封装内 nn2opt）→ 大罚项鼓励同区块连续（近似实现，非硬约束证明）
```
