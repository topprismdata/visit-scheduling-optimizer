...
| v4 微调 `same_weekday_only=True` | ⚠ 仅保证星期几不换，**不保证双周相位合法**（可跨周移动 → 破相位合同，见下方 W53/边界）；微调场景专用，勿与主线混用 |

## 边界警告与整改方向

- **W53 断裂与整改方向（2026-09-07 评审，VisitIR 不变量候选）**：ISO 周 mod 2 相位在 53 周年（如 2026）边界 parity 断裂。整改方向是语义层直接定义连续周锚点相位 `phase(d) = ⌊(weekStart(d) − anchorWeek) / 7天⌋ mod 2`，ISO 周号只作显示/输入映射——**锚点相位是本体（ontology），ISO 周号是表示（representation）**。当前实现仍用 ISO mod 2，跨月使用前必须做锚点实验。
