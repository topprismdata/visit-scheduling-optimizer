...
1. A **set-partitioning master problem with dual-guided column generation** that achieves LP-bounded optimality for each salesperson's 20-day schedule, with column cost computed from the **exact Held–Karp tour length** (no surrogate objective);
...
All discovered negative-reduced-cost columns are added to $\mathcal{P}$ (capped at 250 per round, prioritized by most-negative $\bar{c}$). The loop terminates when no negative-reduced-cost column exists; at that point the LP objective is a **valid lower bound** on the master problem's optimum. A final MIP solve (CP-SAT, time-limit 300 s) enforces integrality; the day-group assignment from the LP-rounded solution is provided as a solution hint.
...
1. **ALNS can violate the work-time cap**: for Rep-6, ALNS produces a day with a 900-minute load — well above the 9-hour cap that the operations data validates. The CG model, by enforcing the time constraint on every column, cannot. In a planning context where the human operation has a hard daily schedule, this is a decisive advantage.
2. **ALNS improves on CG on two instances** (Rep-1, Rep-4). These are the people with the lowest spatial dispersion in the data — the metaheuristic's neighborhood moves happen to find better local minima than the LP-derived dual signal. This is the standard trade-off between exact and metaheuristic approaches and motivates a hybrid for production deployment (e.g., ALNS as a warm-start or repair operator for the IP solver).
...
- **LP lower bounds**: in the distance (open-caliber) model, six of seven salespeople achieve LP-bound = IP-solution, certifying global optimality over the column pool. The closed-loop model is FEASIBLE for four of seven with residual gap 0.6 %–4.2 % from the LP bound.
...
This paper proposes a practical and certifiable method for periodic field-sales visit scheduling that (i) uses the operation's own time data to calibrate per-county travel times and per-visit dwell penalties, (ii) solves the per-salesperson PVRP as a set-partitioning master with dual-guided column generation, and (iii) flattens daily workloads without rerouting. Across 7 salespeople and 235 customers, the method reduces in-day service time by 26 % and active working days by 16 % compared to actual human routes, with LP-certified global optimality in 6 of 7 instances in the distance caliber. A Røpke–Pisinger-style ALNS baseline under identical constraints and budget finds comparable or slightly better solutions on 2 of 7 instances (the low-dispersion ones) but violates the 9-hour daily cap in one. The emergent cross-county structure matches the human operation's behavior without enforcing it. The method is reproducible from the operations data and the open-source artifacts shipped with this paper.
