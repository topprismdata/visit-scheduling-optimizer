...
### 12.9 CG 深度收敛扫描（2026-09-08）

新增构造参数 `cg_max_iters`（默认 15）。08/06 两线扫到 100 轮: **incumbent 与 15 轮完全一致**
（673.782 / 140.908），池 1080/1585 列，LP 早已稳定——**−5.47% 即 bp@GRASP 平台期**。

结论: 进一步改善的瓶颈不在定价轮数，在树搜索（分支规则/下潜深度）或与 r2_alns 列的组合。
`cg_max_iters` 参数保留（对强对偶线有冗余安全边际）。
