...
### 10.1 欠账恢复律与适配的干预性质 (补充, 同日实测)

`experiments/validate_projection.py --part E`, W1 计划未执行店 15,055:
...
| 恢复时同星期 | 29% (随机基线 18%) |
| 恢复时换星期 | **71%** |
| 延迟 次周/2周后/3周后 | 25% / 29% / 37% |

**推论**: 业代**不追欠账**; 欠账插入式适配不是模拟业代行为, 而是**管理干预**。
因此:
- 适配的价值不能用"与实际对齐"(预测指标)衡量 → 观测数据上**不可验证**, 需 **A/B**(干预线 vs 对照线)
- 预测类任务用计划本身 (店→星期 0.776), 不用后验投影
- 适配的 KPI = 义务合规率(合同拜访达成), 不是 Jaccard

### 10.2 适配的正确 KPI: 义务合规率 (取代 Jaccard)

`orchestration/experience/compliance.py`, 8月 529 线实测:

| KPI | 均值 | 中位 |
|---|---|---|
| 义务合规率 `Σ min(实际,计划)/计划 / 门店数` | **0.661** | 0.697 |
| 相位合规率 (双周合同相位是义务) | **0.587** | 0.599 |
| 同日执行率 (参考) | 42% | — |

定义: 超访不加分、缺访受罚; 相位命中 = 实际到访周相位(0=偶周/1=奇周)包含计划相位。
**用途**: 适配/干预的效果只在此 KPI 上评估 (A/B: 干预线 vs 对照线), 对齐度(Jaccard)不得作为适配指标。

### 10.3 A/B 可行性: 样本量测算 (决定适配能否被验证)

8月 529 线合规率 σ=0.230 (线间), 两样本 t 检验 α=0.05/80% power:
...
全部可用线数 529 → 每臂 264 → **最小可检出 Δ = 0.056 (5.6pt 合规率)**。
ICC(线内) = 0.242, 门店级 Deff = 53.9 → 有效样本 ≈ 2,152: **随机单位必须是线**(干预作用于业代), 门店级分析仅小幅提效。
→ 结论: 若期望适配干预能把合规率拉起 ≥8pt, A/B 可行 (130/臂); 若期望仅 3-5pt, 观测上不可验证。
