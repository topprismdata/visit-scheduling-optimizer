...
### 附录 B 续：全办 10 线推广（2026-09-05，绝对能力口径）

池 = 每线 4×150s v3 + 2×150s hgs + greedy + nn2opt + cpsat + 2×900s v3 深波浪；SP+CG 按线运行（~3 min/线）。

| 线 | v3 最优 | SP+CG | vs v3 最优 | 认证 gap |
|:---:|:---:|:---:|:---:|:---:|
| 02 | 174.5 | 174.5 | 0.00% | 0.00% |
| 03 | 110.9 | 110.6 | −0.27% | 0.43% |
| 04 | 181.1 | 180.8 | −0.17% | 1.50% |
| 05 | 228.4 | 228.3 | −0.04% | 0.00% |
| 06 | 70.8 | 70.2 | −0.85% | 0.45% |
| 07 | 270.7 | **252.0** | **−6.91%** | 0.23% |
| 08 | 378.1 | 373.4 | −1.24% | 2.18% |
| 09 | 263.2 | 256.9 | −2.39% | 1.22% |
| 10 | 255.9 | 254.5 | −0.55% | 0.05% |
| 11 | 714.8 | 701.2 | −1.90% | 0.86% |
| **Σ** | **2,648.4** | **2,602.4** | **−1.74%** | — |

- **10/10 线不劣于 v3 最优单解**（[META] 结构保证兑现），7/10 线严格改进，线 07 −6.91%；
- 全部给出 LP 认证下界（gap ≤ 2.18%）；原始数据 `output/sp_all_lines_summary.json` + `output/bench_20260905.csv`；
- 复现：`bash run_sp_all.sh pool|deep|sp`（runner 已参数化 `--line`）。
