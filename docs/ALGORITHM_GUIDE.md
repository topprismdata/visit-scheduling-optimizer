...
| # | 算法 | 类型 | 决策层 | 09线里程 | 求解时间 | 最优性保证 |
|---|---|---|---|---|---|---|
| 1 | NN+2-opt（最近邻+2-opt） | 启发式 | ①日内 | 381.7 km | <1 s | 无（基线锚点） |
| 2 | CP-SAT 精确 TSP | 精确 | ①日内 | 326.6 km | ~2 s/天 | ✅ 矩阵上可证最优 |
| 3 | ALNS（自适应大邻域搜索） | 元启发式 | ①+②跨日 | **290.1 km** | ~300 s | 无（实践最优·主力） |
| 4 | LKH-3（Lin-Kernighan-Helsgaun） | 启发式 | ①日内 | 15.97 km/23店¹ | ~5 min/线 | 无（近似比极优） |
| 6 | Ensemble SP（集合划分重组合） | 精确组合 | 组合层 | 290.3 km | <1 s | ✅ 池内最优 |
| 7 | SDR Exact（列生成+LP 下界） | 精确+证书 | 组合层 | 290.3 km | <1 s | ✅ 池内最优 + gap 证书 |
...
| 维度 | 评价 |
|---|---|
| 时间复杂度 | TSP 是 NP-难问题，最坏指数级；**实践**中本项目单日 ≤35 店约 2 秒内证明最优 |
...
### 2.7 在我们场景的表现
- **海珠荔湾09线**：日内重排 **326.6 km**（NN+2-opt 为 381.7 km，再省 14.4%），全线 **~2.7 秒**（单日约 2 秒）；
...
| 维度 | 评价 |
|---|---|
| 时间复杂度 | O(迭代次数 × 单算子成本)；本项目单算子成本 O(n²)（重排受影响日），20,000 次迭代 ≈ 300 秒 |
...
| 维度 | 评价 |
|---|---|
| 时间复杂度 | 实践近线性于节点数（候选边稀疏化），最坏指数级；百万级节点分钟内可得近优解 |
...
| 维度 | 评价 |
|---|---|
| 时间复杂度 | 模型规模 = O(天数 × 池内路线数)；本项目 23 天 × 数百条路线，CP-SAT 毫秒–秒级解出 |
...
| 维度 | 评价 |
|---|---|
| 时间复杂度 | 组合阶段毫秒–秒级；池生成阶段与各生成算法同阶（可离线并行）；LP 阶段多项式时间 |
...
| 维度 | NN+2-opt | CP-SAT | ALNS | LKH-3 | ~~Clustered~~ | Ensemble SP | SDR Exact |
|---|---|---|---|---|---|---|---|
| 09线里程 | 381.7 | 326.6 | **290.1**¹ | —² | ~~374.2~~ | 290.3 | 290.3 |
| 求解时间 | <1 s | ~2 s/天 | ~300 s | ~5 min/线 | 同底层 | <1 s | <1 s |
...
```text
里程 (km)
 400 ┤ NN+2-opt ●381.7                    (横轴=里程，越左越省)
 374 ┤  ~~Clustered ●374.2~~  ❌否决(路边店场景+14.6%, 见§5.7)
 330 ┤   CP-SAT ●326.6                    (无约束矩阵最优)
 300 ┤                      ALNS ●290.1   (换日重分配，主力最优)
 290 ┤   Ensemble/SDR ●290.3 (秒级, 池内最优)
     └──┬──────────┬──────────┬──────┬────→ 求解时间
       <1s        ~2s        ~5min   ~5min+
     SP/SDR     CP-SAT      ALNS    LKH
```
...
| 日常批量月度计划（默认模式） | CP-SAT → ALNS → Ensemble SP → SDR Exact | 全流水线 ~30 min/线，gap 证书齐备 |
...
2. ✅ 可以说："10 条线路、11 名业务代表实测总里程从 16,857 km 降至 3,319 km（−80%），频次合规 100%。"
