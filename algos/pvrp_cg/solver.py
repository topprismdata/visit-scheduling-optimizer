"""
Set-partitioning master with dual-guided *heuristic* column generation.
...
Algorithm (heuristic, NOT exact Branch-and-Price)
---------
...
2. HEURISTIC CG LOOP (≤ CG_ROUNDS, dual-guided greedy pricing):
...
LP value semantics
------------------
The LP solved in step 2a contains the column-restricted master with:
- one-column-per-day, coverage, linking (the master)
- inter-visit gap constraints (linear, added in this implementation)
- daily-cap (only in the closed-loop and time calibers; open is unconstrained)

The pricing problem is NOT solved exactly. Therefore lp_obj is:
- a LOWER BOUND for the restricted master over the columns found
- NOT a global lower bound for the full PVRP
- the gap (MIP - LP) is the residual dual bound loss from heuristic pricing
  + the integer gap of the final IP
...
def _col_cost_closed(
    D: list[list[float]], t0: Sequence[float], idxs: Sequence[int]
) -> float:
    if not idxs:
        return 0.0
    if len(idxs) == 1:
...
def _col_cost_time(
    T: list[list[float]], t0: Sequence[float], svc: Sequence[float], idxs: Sequence[int]
) -> float:
    if not idxs:
        return 0.0
    if len(idxs) == 1:
...
# ---------------------------------------------------------------------------
def _lp_duals(
    pool: Sequence[frozenset],
    costs: Sequence[float],
    days: int,
    freq: Sequence[int],
    pool_idx: dict,
) -> tuple[list | None, list | None, float | None, str]:
    """LP-solve restricted master (with gap constraints) for duals.
    Returns (pi, mu, lp_obj, status). The returned lp_obj is a lower bound
    for the restricted master; NOT a global lower bound for the full
    PVRP (because pricing is heuristic)."""
    n = len(freq)
    ...
    day_cons = [
        solver.Add(sum(w[gi, d] for gi in range(len(pool))) <= 1.0) for d in range(days)
    ]
    lcons = {}
    for i in range(n):
        for d in range(days):
    ...
    # Inter-visit gap constraints (linear, go directly in LP)
    # For each customer i with freq[i] >= 2, the minimum gap between
    # consecutive visits is ⌊days / (freq[i]+1)⌋. We add pairwise
    # constraints for all (d1, d2) pairs with 0 < d2-d1 < gap_i.
    for i in range(n):
        if freq[i] >= 2:
            gap_i = max(1, days // (freq[i] + 1))
            for d1 in range(days):
                for d2 in range(d1 + 1, d1 + gap_i):
                    if d2 < days:
                        solver.Add(x[i, d1] + x[i, d2] <= 1)

    solver.Minimize(
        sum(costs[gi] * w[gi, d] for gi in range(len(pool)) for d in range(days))
    )
    st = solver.Solve()
...
def _mip_solve(
    ...
    if daily_cap is not None and t0 is not None and closed:
        for d in range(days):
            m.Add(
                sum(int(costs[gi] * 100) * w[gi, d] for gi in range(len(pool)))
                <= int(daily_cap * 100)
            )
    m.Minimize(
        sum(
            int(costs[gi] * 100) * w[gi, d]
            for gi in range(len(pool))
            for d in range(days)
        )
    )
...
def _balance(
    ...
    if st in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        return (
            balanced,
            sum(gcosts[frozenset(a)] for a in balanced if a),
            True,
            [gcosts[frozenset(a)] for a in balanced if a],
        )
...
def _solve_core(
    ...
    for rnd in range(CG_ROUNDS):
        ...
        if verbose:
            print(
                f"  CG{rnd}: +{added}列, 池={len(pool_list)}, LP={lp_obj:.0f}",
                flush=True,
            )
