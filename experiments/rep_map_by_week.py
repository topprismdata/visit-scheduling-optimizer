...
function draw(){
  ...
  if (!corrMode) { const rs=document.getElementById('roadstat'); if(rs) rs.innerHTML=''; }
  if (corrMode) {
    document.getElementById('stat').innerHTML +=
...
const ROAD_API = 'http://127.0.0.1:8778/corridor';
const roadCache = {};
function drawLines(mL, mR, daysP, daysA, tag){
  const b=[];
  const drawDays=(layer, days)=>{
    days.forEach(([dow, pts])=>{
      const col=C[dow]||C[6];
      if (tag==='road'){
        // 沿路几何: 只画线(不画顶点), 提高简化度, 保证底图看得见
        L.polyline(pts,{color:col,weight:3,opacity:.85,smoothFactor:4}).addTo(layer);
        const n=pts.length;
        [0, Math.floor(n/2), n-1].forEach(k=>{ if(pts[k]) L.circleMarker(pts[k],{radius:3,color:'#0b0d12',weight:.6,fillColor:col,fillOpacity:1}).addTo(layer); });
        pts.forEach(p=>b.push(p));
      } else {
        L.polyline(pts,{color:col,weight:2,opacity:.9,dashArray:'4,4'}).addTo(layer);
        pts.forEach(p=>{ L.circleMarker(p,{radius:2.2,color:'#0b0d12',weight:.4,fillColor:col,fillOpacity:.9}).addTo(layer); b.push(p); });
      }
    });
  };
  drawDays(mL, daysP||[]); drawDays(mR, daysA||[]);
  return b;
}
function renderCorridor(d, m){
  const w = document.getElementById('weekf').value;
  const line = d.line, key = line+'_'+w;
  if(mL){mL.remove(); mR.remove();}
  mL=L.map('mL',{zoomControl:true,attributionControl:false});
  mR=L.map('mR',{zoomControl:false,attributionControl:false});
  let sL='', sR='';
  ...
  if (cachedRoad) { use(cachedRoad); }
  else fetch(`${ROAD_API}?line=${line}&scope=${w}`).then(r=>r.json()).then(j=>{
    if (j && !j.error) { roadCache[key]=j; use(j); }
    else setTag(`走廊线：<span style="color:#fca5a5">沿路几何获取失败，当前为直线版</span>`);
  }).catch(()=> setTag(`走廊线：<span style="color:#fca5a5">路网服务未启动（直线版）</span>　<span style="color:#8b93a7">启动命令：python experiments/road_server.py</span>`));
  let lock=false;
  mL.on('move zoom',()=>{if(lock)return;lock=true;mR.setView(mL.getCenter(),mL.getZoom(),{animate:false});lock=false;});
  mR.on('move zoom',()=>{if(lock)return;lock=true;mL.setView(mR.getCenter(),mR.getZoom(),{animate:false});lock=false;});
}
